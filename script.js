          ...
          if (response.ok) {
            try {
              ...
              if (isLikelyGenericFallbackSolution(solution)) {
                lastError = new Error("Model returned a generic fallback answer. Set a vision-capable model/server and retry.");
                continue;
              }
              return solution;
            } catch (error) {
              lastError = error instanceof Error ? error : new Error("Invalid model response");
              continue;
            }
          }
